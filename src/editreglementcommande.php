...
<html>
   ...
   <body>
      <?php 
         ...
         $sqlidcmd = "SELECT * FROM `commande` WHERE id_commande = $_GET[commande]";
      ...
      <form class="form" method="post" name="" >
         ...
         <input type="text" class="reglement-input" name="numcheque"  onkeypress="validate_integer(event)" maxlength="12" size="20" placeholder="Numéro de chèque" id="Inputchequenum"  value="<?php echo $rowid[NumChèque];?>" required/>
         <input type="text" class="reglement-input" name="nombanque"  maxlength="30" size="35" placeholder="Nom de la banque" id="Inputchequenom" value="<?php echo $rowid[NomBanque];?>"required/>
         ...
         <input type="text" class="reglement-input" name="numcarte"  onkeypress="validate_integer(event)" maxlength="12" size="20" placeholder="Numéro de la carte" id="Inputcartenum" value="<?php echo $rowid[NumCarte];?>" readonly/>
         <label>Date d'expiration de la carte</label>
         <input type="date" class="individu-input" name="dateexpiration"   min="<?php echo $datetoday?>" maxlength="15" size="20" id="Inputcartedate" value="<?php echo $rowid[DateExpi];?>"readonly/>
         <input type="radio" name="anomalie" value="cbinvalide" id="Inputcarteano" disabled/>CB invalide
         </br></br>
         <label>Montant du règlement : </label>
         <input type="text" maxlength="10" class="article-input"  name="montant" onkeypress="validate_decimal(event)" pattern="(0\.((0[1-9]{1})|([1-9]{1}([0-9]{1})?)))|(([1-9]+[0-9]*)(\.([0-9]{1,2}))?)" step=".01" value="<?php echo $rowid[MontantRèglement];?>" required/>
            ...
            <?php 
               ...
               if(isset($_POST['règler'])){
                 ...
                 switch($_POST['anomalie']) {
                   ...
                   if($_POST['anomalie']){
                       $sqldelete = "Delete FROM `anomalie` where id_commande=$rowid[id_commande]";
                   ...
                   if($anomalie == false){
                       $statut = "Valide";
                       $sqlresolue = "UPDATE `anomalie` SET résolue=1 WHERE id_commande=$rowid[id_commande]";
                       $resultresolue = $con->query($sqlresolue);
               ...
               setTimeout('Redirect()', 5000);   
                  ...
                  //en fonction de sur quel bouton radio chèque ou carte bancaire on a choisi on prépare la requete d'update de la commande 
                   switch($_POST['radio']) {
                   case "Chèque":
                        $sql = "UPDATE `commande` SET statut='$statut', Chèque=1,NumChèque=$numcheque, NomBanque='$nombanque', Carte=0,NumCarte=NULL, DateExpi=NULL,MontantRèglement=$montantreglement  WHERE id_commande=$rowid[id_commande]";
                        //echo $sql;
                       break;
                   case "Carte bancaire":
                       $sql = "UPDATE `commande` SET statut='$statut',Carte=1,NumCarte=$numcarte, DateExpi='$dateexpi', Chèque=0,NumChèque=NULL, NomBanque=NULL, MontantRèglement=$montantreglement  WHERE id_commande=$rowid[id_commande]";
                        //echo $sql;
                       break;
